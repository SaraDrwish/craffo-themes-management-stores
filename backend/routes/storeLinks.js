...
// GET store links with optional limit & ordering
router.get('/', async (req, res) => {
  const { theme_id, platform, search, plan, limit } = req.query;
  let sql = `
  ...
  sql += ` ORDER BY sl.id DESC`; // أحدث المتاجر أولاً
  ...
  const linksWithImageUrl = links.map(link => ({ ...link, image_url: link.image ? `${baseUrl}${link.image}` : null }));
...
router.post('/', verifyAdmin, (req, res) => {
  upload.single('image')(req, res, async (err) => {
    ...
    try {
      ...
      if (!themeExists) { if (req.file) fs.unlinkSync(req.file.path); return res.status(404).json({ error: 'الثيم غير موجود' }); }
      const existing = await get('SELECT id FROM store_links WHERE store_url = ?', [store_url]);
      if (existing) { if (req.file) fs.unlinkSync(req.file.path); return res.status(409).json({ error: 'هذا الرابط موجود مسبقاً' }); }
...
router.put('/:id', verifyAdmin, (req, res) => {
  upload.single('image')(req, res, async (err) => {
    ...
    if (!current) { if (req.file) fs.unlinkSync(req.file.path); return res.status(404).json({ error: 'المتجر غير موجود' }); }
    if (store_url && store_url !== current.store_url) {
      if (!isValidUrl(store_url)) { if (req.file) fs.unlinkSync(req.file.path); return res.status(400).json({ error: 'الرابط غير صالح' }); }
      const existing = await get('SELECT id FROM store_links WHERE store_url = ? AND id != ?', [store_url, req.params.id]);
      if (existing) { if (req.file) fs.unlinkSync(req.file.path); return res.status(409).json({ error: 'الرابط موجود مسبقاً' }); }
    }
    if (theme_id && theme_id !== current.theme_id) {
      const themeExists = await get('SELECT id FROM themes WHERE id = ?', [theme_id]);
      if (!themeExists) { if (req.file) fs.unlinkSync(req.file.path); return res.status(404).json({ error: 'الثيم الجديد غير موجود' }); }
    }
    let imagePath = current.image;
    if (req.file) {
      if (current.image && fs.existsSync(path.join(__dirname, '..', current.image))) fs.unlinkSync(path.join(__dirname, '..', current.image));
...
router.delete('/:id', verifyAdmin, async (req, res) => {
  ...
  if (store && store.image && fs.existsSync(path.join(__dirname, '..', store.image))) fs.unlinkSync(path.join(__dirname, '..', store.image));
