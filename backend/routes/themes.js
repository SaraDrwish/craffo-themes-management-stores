...
router.get('/:id', async (req, res) => {
  try {
    ...
    const stores = await all(`
      SELECT id, store_name, store_url, platform, plan
      FROM store_links
      WHERE theme_id = ? AND is_approved = 1
...
router.post('/:id/reset-api', verifyAdmin, async (req, res) => {
  try {
    ...
    await run(`
      ...
      WHERE id=?
    `, [original.name, original.image, original.description, original.price, original.demo_url, original.purchase_url, original.plan || 'starter', req.params.id]);
    res.json({ success: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
