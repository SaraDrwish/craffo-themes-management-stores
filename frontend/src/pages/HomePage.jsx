...
import { getAllThemes, getAllCategories } from '../services/api';
...
export default function HomePage() {
  ...
  useEffect(() => {
    loadThemes();
  }, [platform, planFilter, categoryFilter]);
  ...
  async function loadThemes() {
    ...
    const data = await getAllThemes(platform, plan, cat);
    setThemes(data);
    setFiltered(data);
    setLoading(false);
  }
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="container mx-auto px-4 py-6 flex-1">
        ...
        <div className="flex justify-center gap-2 mb-6 flex-wrap">
          <button onClick={() => setCategoryFilter('')} className={`px-3 py-1 rounded-full text-sm ${!categoryFilter ? 'bg-purple text-white' : 'bg-gray-200'}`}>كل الفئات</button>
          {categories.map(cat => (
            <button key={cat.id} onClick={() => setCategoryFilter(cat.name)} className={`px-3 py-1 rounded-full text-sm ${categoryFilter === cat.name ? 'bg-purple text-white' : 'bg-gray-200'}`}>
              {cat.name}
            </button>
          ))}
        </div>
