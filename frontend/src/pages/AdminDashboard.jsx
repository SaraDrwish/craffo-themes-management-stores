...
export default function AdminDashboard() {
  ...
  const handleThemeDelete = async (id) => {
    if (confirm('حذف الثيم؟')) { await deleteTheme(id, token); fetchThemes(); fetchStats(); }
  };

  const handleThemeReset = async (id) => {
    if (confirm('استعادة بيانات API؟')) { await resetThemeApi(id, token); fetchThemes(); }
  };

  const handleThemeReorder = async (id, direction) => {
    await reorderTheme(id, direction, token);
    fetchThemes();
  };
  ...
  const handleCategoryDelete = async (id) => {
    if (confirm('حذف الفئة؟')) { await deleteCategory(id, token); fetchCategories(); }
  };
  ...
  const handleStoreLinkDelete = async (id) => {
    if (confirm('حذف المتجر؟')) { await deleteStoreLink(id, token); fetchStoreLinks(); }
  };
  ...
  const renderStoreLinkForm = () => (
    <div className="bg-white p-4 rounded-lg mb-4">
      ...
      <select value={formData.platform || 'Salla'} onChange={e => setFormData({...formData, platform: e.target.value})} className="border p-1 rounded w-full mb-2"><option value="Salla">سلة</option><option value="Zid">زد</option></select>
      ...
      <div className="p-4">
          ...
          <>
            ...
            <div className="bg-white rounded shadow overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-200"><tr><th>الاسم</th><th>المنصة</th><th>الباقة</th><th>المتاجر</th><th></th></tr></thead>
                <tbody>
                  {themes.map(t => (
                    <tr key={t.id} className="border-b">
                      <td className="p-2">{t.name}</td><td>{t.platform}</td><td>{t.plan}</td><td>{t.stores_count}</td>
                      <td className="flex gap-1">
                        <button onClick={() => { setEditingTheme(t); setFormData(t); }} className="bg-blue-500 text-white px-2 py-0.5 rounded text-xs">تعديل</button>
        ...
        {activeTab === 'categories' && (
          <>
            {renderCategoryForm()}
            <div className="bg-white rounded shadow overflow-x-auto">
              <table className="w-full text-sm"><thead className="bg-gray-200"><tr><th>اسم الفئة</th><th>الثيم</th><th>عدد المتاجر</th><th></th></tr></thead>
              <tbody>
                {categories.map(c => (
                  <tr key={c.id} className="border-b">
                    <td className="p-2">{c.name}</td><td>{themes.find(t => t.id === c.theme_id)?.name}</td><td>{c.stores_count}</td>
                    <td><button onClick={() => { setEditingCategory(c); setFormData({ theme_id: c.theme_id, name: c.name }); }} className="bg-blue-500 text-white px-2 py-0.5 rounded text-xs">تعديل</button>
                    <button onClick={() => handleCategoryDelete(c.id)} className="bg-red-500 text-white px-2 py-0.5 rounded text-xs ml-1">حذف</button></td>
                  </tr>
                ))}
              </tbody></table>
            </div>
        ...
        {activeTab === 'stores' && (
          <>
            {renderStoreLinkForm()}
            <div className="bg-white rounded shadow overflow-x-auto">
              <table className="w-full text-sm"><thead className="bg-gray-200"><tr><th>اسم المتجر</th><th>الرابط</th><th>الفئة</th><th>المنصة</th><th></th></tr></thead>
              <tbody>
                {storeLinks.map(s => (
                  <tr key={s.id} className="border-b">
                    <td className="p-2">{s.store_name}</td><td className="truncate max-w-xs">{s.store_url}</td><td>{s.category_name}</td><td>{s.platform}</td>
                    <td><button onClick={() => { setEditingStoreLink(s); setFormData({ category_id: s.category_id, store_name: s.store_name, store_url: s.store_url, platform: s.platform }); }} className="bg-blue-500 text-white px-2 py-0.5 rounded text-xs">تعديل</button>
                    <button onClick={() => handleStoreLinkDelete(s.id)} className="bg-red-500 text-white px-2 py-0.5 rounded text-xs ml-1">حذف</button></td>
                  </tr>
                ))}
              </tbody></table>
            </div>
