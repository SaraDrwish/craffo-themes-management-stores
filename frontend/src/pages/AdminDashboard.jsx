...
export default function AdminDashboard() {
  ...
  // تحديث قائمة الثيمات عند تغيير المنصة في النموذج
  useEffect(() => {
    if (formData.platform) {
      const filtered = themes.filter(t => t.platform === formData.platform);
      setFilteredThemes(filtered);
      // إعادة تعيين theme_id إذا كان غير متوافق
      if (formData.theme_id && !filtered.some(t => t.id == formData.theme_id)) {
        setFormData(prev => ({ ...prev, theme_id: '' }));
      }
    } else {
      setFilteredThemes(themes);
    }
  }, [formData.platform, themes]);
  ...
  const handleStoreSave = async () => {
    ...
    else payload.append('image', DEFAULT_STORE_IMAGE);
    try {
  ...
  const renderStoreForm = () => {
    const themeOptions = filteredThemes.map(t => ({ value: t.id, label: t.name }));
      ...
      <div className="bg-white p-4 rounded-lg mb-4">
        ...
        <div className="space-y-3">
          ...
          <SearchableSelect
            ...
            onChange={(val) => setFormData({...formData, theme_id: val})}
  ...
  const startBulkImport = async () => {
    if (!bulkUrls.trim()) { alert('الرجاء إدخال روابط'); return; }
    if (!selectedBulkThemeId) { alert('الرجاء اختيار ثيم للمتاجر المضافة'); return; }
    setIsImporting(true);
    setImportStop(false);
    ...
    for (let line of lines) {
      line = line.trim();
      if (line && !line.startsWith('#') && line.match(/^https?:\/\/.+/i)) {
        urls.push(line);
      }
    }
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      try {
        ...
        if ((i+1) % 5 === 0 || i === total-1) await fetchStoreLinks();
  ...
  const stopBulkImport = () => setImportStop(true);
  ...
  const renderBulkImport = () => (
    <div className="bg-white p-4 rounded-lg mb-4 border-2 border-dashed border-purple-300">
      ...
      <p className="text-sm text-gray-600 mb-2">أدخل رابطًا واحدًا في كل سطر (أو سطر يبدأ بـ # كتعليق).</p>
      <textarea rows={6} className="border p-2 rounded w-full font-mono text-sm" value={bulkUrls} onChange={(e) => setBulkUrls(e.target.value)} disabled={isImporting} />
      <div className="flex flex-wrap gap-3 mt-3 items-center">
        <select value={selectedBulkThemeId} onChange={(e) => setSelectedBulkThemeId(e.target.value)} className="border p-2 rounded" disabled={isImporting}>
          <option value="">-- اختر ثيم لكل المتاجر --</option>
          {themes.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
        </select>
        {!isImporting ? <button onClick={startBulkImport} className="bg-blue-500 text-white px-4 py-2 rounded">بدء الإضافة المجمعة</button>
          : <button onClick={stopBulkImport} className="bg-red-500 text-white px-4 py-2 rounded">إيقاف الطابور</button>}
      </div>
      {isImporting && importProgress.total > 0 && (
        <div className="mt-3"><div className="w-full bg-gray-200 rounded-full h-2.5"><div className="bg-purple-600 h-2.5 rounded-full" style={{ width: `${(importProgress.current / importProgress.total) * 100}%` }}></div></div>
        <p className="text-sm text-gray-600 mt-1">تمت إضافة {importProgress.current} من {importProgress.total} رابط</p></div>
      )}
    </div>
  );
      ...
      <div className="p-4">
        ...
        {activeTab === 'stores' && (
          <>
            ...
            {renderBulkImport()}
            <div className="bg-white rounded shadow overflow-x-auto">
              <table className="w-full text-sm"><thead className="bg-gray-200">…<th>اسم المتجر</th><th>الرابط</th><th>الثيم</th><th>المنصة</th><th>الباقة</th><th>الصورة</th><th></th></thead><tbody>{storeLinks.map(s => {
                const themeName = themes.find(t => t.id === s.theme_id)?.name || 'غير معروف';
                return (<tr key={s.id} className="border-b"><td className="p-2">{s.store_name}</td><td className="truncate max-w-xs">{s.store_url}</td><td>{themeName}</td><td>{s.platform}</td><td>{s.plan === 'starter' ? '🚀 انطلاق' : s.plan === 'growth' ? '🌟 نمو' : s.plan === 'gold' ? '👑 ذهبية' : 'بدون باقة'}</td><td>{s.image_url ? <img src={s.image_url} className="w-8 h-8 object-cover rounded" /> : 'لا توجد'}</td><td><button onClick={() => { setEditingStoreLink(s); setFormData({ theme_id: s.theme_id, store_name: s.store_name, store_url: s.store_url, platform: s.platform, plan: s.plan || 'none' }); setPreviewUrl(null); }} className="bg-blue-500 text-white px-2 py-0.5 rounded text-xs">تعديل</button><button onClick={() => handleStoreDelete(s.id)} className="bg-red-500 text-white px-2 py-0.5 rounded text-xs">حذف</button></td></tr>); })}</tbody></table>
            </div>
          </>
        )}
      </div>
