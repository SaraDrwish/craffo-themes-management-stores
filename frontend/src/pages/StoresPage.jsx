...
export default function StoresPage() {
  ...
  useEffect(() => {
    let filtered = [...stores];
    if (selectedThemeId) {
      filtered = filtered.filter(store => store.theme_id === parseInt(selectedThemeId));
    }
    if (planFilter !== 'all') {
      filtered = filtered.filter(store => store.plan === planFilter);
    }
    if (search.trim()) {
      const lowerSearch = search.toLowerCase();
      filtered = filtered.filter(store =>
        store.store_name.toLowerCase().includes(lowerSearch) ||
        store.store_url.toLowerCase().includes(lowerSearch) ||
      ...
      <div className="container mx-auto px-4 py-6 flex-1">
        ...
        <div className="flex justify-center gap-2 mb-4 flex-wrap">
          <button
            ...
            className={`px-4 py-1 rounded-full text-sm ${!selectedThemeId ? 'bg-dark-navy text-white' : 'bg-gray-200'}`}
          >
            كل الثيمات
          </button>
          {themes.map(theme => (
            <button
              ...
              className={`px-4 py-1 rounded-full text-sm ${selectedThemeId == theme.id ? 'bg-dark-navy text-white' : 'bg-gray-200'}`}
            >
              {theme.name}
            </button>
          ))}
        ...
        <div className="flex justify-center gap-3 mb-6 flex-wrap">
          {[
            ...
            <button
              ...
              className={`px-4 py-1 rounded-full text-sm ${planFilter === plan.value ? 'bg-dark-navy text-white' : 'bg-gray-200'}`}
            >
              {plan.label}
            </button>
          ))}
        ...
        <div className="flex justify-center">
          <input
            ...
            className="w-full max-w-md border border-purplelight rounded-full px-4 py-2"
          />
        </div>
